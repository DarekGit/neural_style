...
def modelSelector(model_file, pooling,debug):
    ...
    if any(name in model_file for name in vgg_list):
        ...
        if debug: print("NIN Architecture Detected")
...
def loadCaffemodel(model_file, pooling, use_gpu, disable_check,debug=True):
    cnn, layerList = modelSelector(str(model_file).lower(), pooling,debug)

    cnn.load_state_dict(torch.load(model_file), strict=(not disable_check))
    if debug: print("Successfully loaded " + str(model_file))
    ...
    if debug: print_loadcaffe(cnn, layerList)
